...
// ../bootstrap-editorconfig/dist/index.mjs
function copyTemplateFile() {
  template_exports.copy(".editorconfig");
}
...
// src/index.ts
var supports = [task];
var packagePath = import_node_path.default.resolve(import_node_process.default.cwd(), "package.json");
var packageContent = import_node_fs.default.readFileSync(packagePath, { encoding: "utf-8" });
var packageJSON = JSON.parse(packageContent);
async function getExecTasks() {
  const response = await question_exports.createQuestion({
    type: "multiselect",
    name: "modules",
    message: "\u8BF7\u9009\u62E9\u9700\u8981\u5B89\u88C5\u7684\u6A21\u5757",
    choices: supports.map((task2) => ({
      title: task2.name,
      value: task2.name,
      selected: true
    })),
  ...
  return supports.filter((task2) => {
    var _a;
    return (_a = response == null ? void 0 : response.modules) == null ? void 0 : _a.includes(task2.name);
  });
}
async function getTaskQuestions(tasks) {
  const questions = tasks.flatMap((task2) => task2.questions);
  return question_exports.createQuestion(questions);
}
async function getTaskExections(tasks, response) {
  return tasks.map((task2) => task2.resolve(response));
}
